...
export class RecommendationService {
  ...
  private static calculateCategoryFrequency(categories: Array<{ name: string }>): CategoryCount[] {
  ...
  private static calculateUserInteractions(userData: {
    id: string;
    reviews: Array<{ brief: { author: { id: string; name: string | null } } }>;
    briefUpvotes: Array<{ brief: { author: { id: string; name: string | null } } }>;
  }): UserInteraction[] {
    ...
    userData.briefUpvotes.forEach((upvote) => {
  ...
  private static calculateCitationDomains(briefs: Array<{ sources?: Array<{ url: string }> }>): DomainCount[] {
  ...
  private static calculateEngagementScore(userData: {
    briefs: unknown[];
    reviews: unknown[];
    briefUpvotes: unknown[];
    savedBriefs: unknown[];
  }): number {
  ...
  private static calculateContentQualityScore(briefs: Array<{ reviews: Array<{ rating: number }> }>): number {
    ...
    briefs.forEach(brief => {
      if (brief.reviews.length > 0) {
        const avgRating = brief.reviews.reduce((sum: number, r) => sum + r.rating, 0) / brief.reviews.length;
