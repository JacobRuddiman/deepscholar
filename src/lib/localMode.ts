 ...
 *
 * Provides mock authentication and session data for local development
 * without requiring real OAuth providers or a production database.
 *
 * Set NEXT_PUBLIC_LOCAL_AUTH=true and/or NEXT_PUBLIC_LOCAL_DB=true in .env
 * to enable local mode.
 */

/** Whether local authentication bypass is enabled */
export const LOCAL_AUTH = process.env.NEXT_PUBLIC_LOCAL_AUTH === 'true';

/** Whether local database mode is enabled */
export const LOCAL_DB = process.env.NEXT_PUBLIC_LOCAL_DB === 'true';

/** Whether any local mode is active (auth or DB) */
...
/** Returns the mock local user if LOCAL_AUTH is enabled, otherwise null */
export function getLocalUser() {
  return LOCAL_AUTH ? LOCAL_USER : null;
}

/** Returns a mock session if LOCAL_AUTH is enabled, otherwise null */
export function getLocalSession() {
  return LOCAL_AUTH ? LOCAL_SESSION : null;
}

/** Whether any local development mode is active */
export function isLocalMode() {
  return LOCAL_MODE;
}

/** Whether local auth bypass is active */
export function isLocalAuth() {
  return LOCAL_AUTH;
}

/** Whether local database mode is active */
